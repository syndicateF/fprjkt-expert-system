...
def detect_conflicts(user_input):
    ...
    for conflict in conflicts:
        ...
        for example in conflict.examples:
            ...
            special_entities = ['divisi_keuangan', 'divisi_acara', 'kebijakan_baru']
            detected_entities = [word for word in special_entities if word in tokens1.union(tokens2)]
            current_entity_boost = 0.3 if detected_entities else 0
            
            sim = (current_base_sim + current_entity_boost)
            ...
            if sim > max_score:
                ...
                debug_info = {
                    'input_tokens': list(tokens1) if tokens1 else [],
                    'example_tokens': list(tokens2) if tokens2 else [],
                    'matched_tokens': list(intersection) if intersection else [],
                    'matched_count': len(intersection),
                    'total_tokens': len(union),
                    'detected_entities': detected_entities if detected_entities else [],
                    'entity_count': len(detected_entities),
                    'relation_type': rel_type if rel_type else 'none',
