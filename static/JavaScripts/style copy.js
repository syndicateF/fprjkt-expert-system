...
function addSolution() {
    ...
    textarea.placeholder = `Solution ${container.childElementCount + 1}`;


    const removeButton = document.createElement('button');
    removeButton.type = 'button';
    removeButton.textContent = 'Remove Solution';
    removeButton.onclick = () => inputDiv.remove();
...
function addCause() {
    ...
    const textarea = document.createElement('textarea');
    textarea.name = 'cause[]';
    textarea.placeholder = `Cause ${container.childElementCount + 0}`;

    const removeButton = document.createElement('button');
    removeButton.type = 'button';
    removeButton.textContent = 'Remove Cause';
    removeButton.onclick = () => inputDiv.remove();
...
function addDependsOn() {
    ...
    const textarea = document.createElement('textarea');
    textarea.name = 'depends_on[]';
    textarea.placeholder = `Deepends on ${container.childElementCount + 0}`;

    const removeButton = document.createElement('button');
    removeButton.type = 'button';
    removeButton.textContent = 'Remove Depends On';
    removeButton.onclick = () => inputDiv.remove(); 
...
function addResolve() {
    ...
    const inputDiv = document.createElement('div');
    const textarea = document.createElement('textarea');
    textarea.name = 'resolve[]';
    textarea.placeholder = `Resolve ${container.childElementCount + 0}`;

    const removeButton = document.createElement('button');
    removeButton.type = 'button';
    removeButton.textContent = 'Remove Resolve';
    removeButton.onclick = () => inputDiv.remove();
...
async function submitRelations(conflictId) {
    ...
    document.querySelectorAll(`#cause-container-${conflictId} textarea[name="cause-${conflictId}[]"]`).forEach((el) => {
        formData.append(`cause-${conflictId}[]`, el.value);
    });
    document.querySelectorAll(`#depends_on-container-${conflictId} textarea[name="depends_on-${conflictId}[]"]`).forEach((el) => {
        formData.append(`depends_on-${conflictId}[]`, el.value);
    });
    document.querySelectorAll(`#resolve-container-${conflictId} textarea[name="resolve-${conflictId}[]"]`).forEach((el) => {
        formData.append(`resolve-${conflictId}[]`, el.value);
    });
    ...
    try {
        ...
        if (response.ok) {
            alert('Relations submitted successfully!');
            location.reload();
        } else {
            const errorData = await response.json();
            alert(`Gagal submit relations: ${errorData.message}`);
        }
    } catch (error) {
